...
subtitle: "An Analysis of Female Candidate Success in Canadian Municipal Election Over Time, with a Focus on Trends and Incumbency."
author: 
  - Hanqing Yang
...
## Paper Structure
The paper is structured as follows: After the introduction in @sec-intro, the data section @sec-data provides an overview of the dataset. The model specification is detailed in @sec-model, where the logistic regression framework and its statistical rationale are explained. The results are presented in @sec-result, highlighting key trends and patterns related to gender and electoral outcomes in Canadian municipal elections. @sec-discussion interprets the findings in depth, within the broader context of gender representation in local politics, explain limitations and offering directions for future research. And appendix @sec-appendix1 give aspect of sampling method and observational data. @sec-data-details provide additional details of data and data cleaning process, @sec-model-details provide the model summary and validation.
...
#| label: tbl-elected
#| tbl-cap: Showing the distribution of candidates' elected status in CMED, including the total number and percentage of those elected and not elected, along with an overall total rows.
#| echo: false
...
```{r}
#| label: fig-elected
#| fig-cap:  Graph showing the distribution of elected status ('Elected' and 'Not Elected') by gender ('Male' and 'Female'), highlighting the relative proportions of male and female candidates in each elected category, with percentages of male/female candidate being elected/not elected annotated.
#| echo: false
#| warning: false
#| message: false
...
```{r,fig.height= 5, fig.width = 8}
#| label: tbl-gender
#| tbl-cap: Showing the distribution of candidates' gender in CMED, including the total number and percentage of female and male, along with an overall total rows.
#| echo: false
...
```{r,fig.height= 5, fig.width = 8}
#| label: tbl-incumbent
#| tbl-cap: Showing the distribution of candidates' incumbency status in CMED, including the total number and percentage of candidates who were incumbent and non-incumbent before election, along with an overall total rows.
#| echo: false
...
```{r}
#| label: fig-incumbent
#| fig-cap: A plot visualizing the relationship between elected status ("Elected" and "Not Elected") and incumbency status ("Not Incumbent" in yellow and "Incumbent" in green), highlighting the distribution of incumbents and non-incumbents across election outcomes.
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| label: fig-year
#| fig-cap:  Showing the number of male and female candidates in Canadian municipal elections over time, highlighting a sharp increase in candidate participation post-2000 and a persistent gender gap, with male candidates consistently outnumbering female candidates."
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| label: tbl-summaryx
#| tbl-cap: Key descriptive statistics for predictor variables, including the counts and proportions of female and male candidates, incumbent and non-incumbent candidates, and the range of election years covered in the dataset.
#| echo: false
#| warning: false
#| message: false
...
#| fig-cap: Showing the proportion of candidates elected over time by gender, with separate lines for male (blue) and female (pink) candidates, highlighting trends in gender representation across election years.
#| echo: false
#| warning: false
#| message: false
...
#| fig-cap: Showing the proportion of elected outcomes ("Elected" and "Not Elected") by gender ("Male" and "Female") for incumbents and non-incumbents, illustrating how incumbency influences election success differently for male and female candidates.
#| echo: false
#| warning: false
#| message: false
...
```
The plot (@fig-result3) displays the predicted probabilities of election for male and female candidates, separated by incumbency status. Among non-incumbents, female candidates show a higher predicted probability of being elected compared to male candidates, highlighting a notable gender advantage in this category. In contrast, for incumbents, the predicted probabilities are more balanced between genders, with female candidates having a slight edge. These results illustrate how incumbency status interacts with gender to influence electoral success, with incumbency narrowing the gender gap observed in non-incumbent candidates. 
...
# Appendix: Observational data {#sec-appendix1}
The CMED used in this study is a comprehensive observational dataset spanning over 150 years of municipal elections across Canada. It compiles candidate-level data from provincial archives, municipal records, and newspapers to capture electoral trends. This appendix examines the dataset's sampling framework, observational nature, and potential biases,and evaluation of its strengths and limitations.
...
## Simulation: Sampling Bias and Validation

```{r}
#| label: tbl-bias
#| fig-cap: Bootstrap summary statistics showing the mean success rates and standard deviations for female candidates in 'Small' municipalities (fewer than 10 female candidates) and 'Large' municipalities, highlighting higher average success rates and lower variability in smaller municipalities compared to larger ones.
#| echo: false
#| warning: false
#| message: false
...
municipality_success <- train_data |>
...
set.seed(1202)  # For reproducibility
...
```{r}
#| label: fig-bias
#| fig-cap: Plot of bootstrap distributions for female candidates' success rates in 'Small' (fewer than 10 female candidates) and 'Large' municipalities, showing higher success rates with lower variability in smaller municipalities and lower success rates with higher variability in larger ones."
#| echo: false
#| warning: false
#| message: false
...
A bootstrap analysis was performed to evaluate sampling biases in the dataset, focusing on how municipality size affects female candidate success rates. Using 1,000 resamples to estimated success rates for "Small" municipalities (fewer than 10 female candidates) and "Large" municipalities separately. This method captures the variability and central tendency of success rates without assuming a specific underlying distribution, making it well-suited for observational data like the CMED.

### Results and Implications
The bootstrap results (@tbl-bias) and (@fig-bias) show that female candidates in small municipalities have higher average success rates compared to those in large municipalities. This suggests that smaller municipalities may provide a more supportive electoral environment, while larger municipalities present more complex and competitive dynamics. These findings indicate the potential for sampling bias, as the overrepresentation of smaller municipalities could skew observed trends in gender-related election success.

The disparities in candidate distribution across municipalities further highlight the potential for sampling bias in the dataset. Since the smallest municipalities hosting far fewer candidates than the largest ones, this imbalance underscores the need to consider municipality size as a factor influencing electoral outcomes and to address potential biases in the dataset. Failure to address these biases could lead to overestimating the overall success rates of female candidates and misinterpreting the broader gender dynamics in Canadian municipal elections.

### Possible Alternative Methodologies
An possible approach for improving the analysis of the CMED dataset would involve weighting to adjust for over- or underrepresentation of specific municipalities or candidate groups. For instance, smaller municipalities, which tend to have disproportionately high success rates for female candidates, could be weighted less to prevent biases in gender-related analyses. Similarly, larger municipalities could receive higher weights to better reflect their influence in overall electoral trends
...
1. **Column Selection and Naming**: The **`janitor::clean_names()`** function was used to standardize column names to a consistent format. Only relevant columns were retained, including `race_id`, `first_name`, `last_name`, `municipality`, `gender`, `elected`, `incumbent`, `election_year`, and `province`.

2. **Variable Transformation**: The `gender` variable originally has value 'M' and 'F' indicating male and female, I used mutate() function to recode it with binary values, with `1` representing female candidates and `0` representing male candidates. The `incumbent` and `elected` columns were converted to factors, ensuring appropriate data types for analysis.

3. **Handling Missing Values and Duplicates**: Observations with missing values were removed using **`tidyr::drop_na()`**, and duplicate rows were eliminated using **`dplyr::distinct()`**, ensuring data consistency.

4. **Factor Level Reordering**: The `elected` variable was reordered with levels `0` (not elected) and `1` (elected) to ensure clarity in logistic regression modeling.

5. **Data Splitting**: The cleaned dataset was randomly split into training (70%) and test (30%) sets using **`rsample::initial_split()`**, with a fixed random seed for reproducibility. These sets formed the basis for model fitting and validation.

6. **Identifying Earliest Year of Female Participation**: The dataset was grouped by gender to identify the first year female candidates participated in elections, guiding the decision to use only data after 1910 to ensure both genders were represented.

7. **Filtering by Year**: To address the absence of female candidates in early years, only elections after 1910 were included in the training and test datasets, ensuring balanced representation for gender-based analysis.

Finally, I saved the cleaned dataset and the training/testing splits in CSV and Parquet formats using [@arrow], facilitating efficient storage and future accessibility.
